      ...
      teinte = 0;
        for(int zy = -2; zy < 3; zy++){
          for(int zx = -2; zx < 3; zx++){
             if(i + zy*PImgWebCam.width >= 0 && i+zy*PImgWebCam.width < PImgWebCam.width*PImgWebCam.height){
               a=zy*PImgWebCam.width;
             }else{
               a=0;
             }
             if(xx + zx >= 0 && xx+zx < PImgWebCam.width){
               b=zx;
             }else{
               b=0;
             }
             teinte += hue(PImgWebCam.pixels[i+a+b]);
          }  
        }
